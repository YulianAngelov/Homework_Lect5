...
//                {13, 14, 15}
        ...
        for (int row = 0; row < matrix.length; row++) {
            for (int col = 0; col < matrix[0].length; col++) {
                if (matrix[row][col] > 9 || matrix[row][col] < 0) {spc = " ";}
                System.out.println( "a[" + row + "," + col + "] = " + spc + matrix[row][col] + "; ");
            }
        }
        System.out.println(" ");
 ...
 /*       do {
            do {
                System.out.println( "a" + row + col + " = " + matrix[row][col] + "; ");
                col++;
            } while (col < matrix[0].length); // do col
            row++;
        } while ( row < matrix.length );   //* row
    }   // printMatrixElements()  */
    }
...
// № 3 ******************************************
    public static void printDynamicMatrixElements() {
        ...
        for (int row = 0; row < matrix.length; row++) {
            if ( row > 0) { Spases = "  ";}
            System.out.print(Spases + "{ ");

            for (int col = 0; col < matrix[0].length; col++) {
                System.out.print(matrix[row][col] + ", ");
            }
            if(cols == matrix[0].length && row < matrix.length -1 ) {
                System.out.println("}");
            }
        }
        System.out.print("} }");
...
// № 4 ******************************************
    public static void printBiggestNum() {
        int[] numbers = {10, 30, 0, 20, 50, 40, -7, 17, 23,44, -2, };

        int biggestNum = numbers[0];

        int i = 0;  // Would be 1 and so do not make first loop, but about print of elements...

        System.out.println( "From these elements:");
        do {
